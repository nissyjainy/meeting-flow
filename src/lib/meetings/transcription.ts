...
export const transcribeMeetingFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (meetingError) {
        transcriptionError("fetch meeting failed", meetingError, { meetingId: data.meetingId });
        throw new Error(meetingError.message);
      }
      if (!meeting) {
        const err = new Error("Meeting not found.");
        transcriptionError("meeting not found", err, { meetingId: data.meetingId });
        throw err;
      }
      if (!meeting.file_url) {
        const err = new Error("Meeting has no file URL.");
        transcriptionError("missing file_url", err, { meetingId: data.meetingId });
        throw err;
      }
      ...
      transcriptionLog("handler success", { meetingId: meeting.id });
